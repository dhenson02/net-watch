...
- `sink_clickhouse::apply_schema` splits the file with `split_statements`, which drops `--` comments and splits on `;` outside quoted strings/identifiers. Keep the file to plain statements that it can split (no `$$`-style bodies, no `/* */` comments containing `;`). The container's init path does not share this splitter, so only the collector, `cargo test schema_splits`, or `cargo test -- --ignored` catches a split problem.
